...
def MLPJaxFactory(network_config = {},
                  train = True):
    return MLPJax(layer_sizes = network_config['layer_sizes'], 
...
class MLPJax(nn.Module):
    ...
    def setup(self):
        ...
        self.activation_funs = [self.activations_dict[activation] for activation in self.activations if (activation != 'linear')]
...
class ModelTrainerJaxMLP:
    def __init__(self,
        ...
        if not ('loss_dict' in config.keys()):
            self.loss_dict = {'huber': {'fun': optax.huber_loss,
                                   'kwargs': {'delta': 1}},
                              'mse': {'fun': optax.l2_loss,
                                     'kwargs': {}},
                              'bcelogit': {'fun': optax.sigmoid_binary_cross_entropy,
                                          'kwargs': {}}
                             }
        else:
        ...
        if not ('lr_dict' in config.keys()):
            ...
            self.lr_dict = {'init_value': 0.0002,
                            'peak_value': 0.02,
                            'end_value': 0.0,
        ...
        self.training_history = 'Please run training for this attribute to be specified!'
        self.state = 'Please run training for this attribute to be specified!'
            
    def __get_loss(self):
        self.loss = partial(self.loss_dict[self.config['loss']]['fun'], **self.loss_dict[self.config['loss']]['kwargs'])
    ...
    def create_train_state(self, rng):
        params = self.model.init(rng, jnp.ones((1, self.train_dl.dataset.input_dim))) # self.config['input_size'])))
        lr_schedule = optax.warmup_cosine_decay_schedule(init_value = self.lr_dict['init_value'],
    ...
    def train_and_evaluate(self):
        ...
        if self.model.train_output_type == 'logprob':
            ...
            print('Model type identified as "unknown" because the training_output_type attribute' +\
                  ' of the supplied jax model is neither "logprob", nor "logits"')
        
        # Initialize Training history
        training_history = pd.DataFrame(np.zeros((self.config['n_epochs'], 2)), columns = ['epoch', 'val_loss'])

        # Initialize network
        if type(self.seed) != int:
            raise ValueError('seed argument is not an integer, '+ \
        ...
        for epoch in range(self.config['n_epochs']):
            ...
            print('Epoch: {} / {}, test_loss: {}'.format(epoch, self.config['n_epochs'], 
                                                         test_loss))
        ...
        # Saving
        full_path = self.output_folder + '/' + self.output_file_id + '_' + self.model_type + '_' + self.run_id 
